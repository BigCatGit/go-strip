...
func readRawBuildInfo(r io.ReaderAt) (vers, mod string, err error) {
	...
	if data[15]&2 != 0 {
		vers, data, index, length = decodeString(data[32:])
		tmpAddr += uint64(index)
		core.TypeStringOffsets.GoMod = append(core.TypeStringOffsets.GoMod, core.TypeData{
			Offset: tmpAddr,
			Length: length,
		})
		tmpAddr += length

		mod, data, index, length = decodeString(data)
		tmpAddr += uint64(index)
		core.TypeStringOffsets.GoMod = append(core.TypeStringOffsets.GoMod, core.TypeData{
			Offset: tmpAddr,
			Length: length,
...
func decodeString(data []byte) (s string, rest []byte, index int, length uint64) {
	...
	index = n
	length = u
	return string(data[n : uint64(n)+u]), data[uint64(n)+u:], index, length
}
