...
func (f *GoFile) init() error {
	...
	f.initPackages.Do(func() {
		...
		offsetFilePath, _ := f.fh.getFva(core.TypeStringOffsets.PCLnTab)
		offset := offsetFilePath + uint64(DataLength-filetab)

		for fileMap.Next() {
			s := fileMap.Key().String()
			value := fileMap.Value().Uint()
			if GoOptions.IsMassup {
				//log.Println("处理文件:", s)
				length := uint64(len(s))
				core.TypeStringOffsets.FileName = append(
					core.TypeStringOffsets.FileName, core.TypeData{
						Offset: offset + value,
						Length: length,
					})
				//fmt.Println(string(f.origin[int(offset+value):int(offset+value+length)]))
			} else {
				//log.Println("File:", s)
			}
		}
		...
		for funcNamesIter.Next() {
			...
			m := value + uint64(len(funcName))
			if maxLength < m {
				maxLength = m
			}
		}
		if GoOptions.IsMassup {
			core.TypeStringOffsets.Func = append(core.TypeStringOffsets.Func, core.TypeData{
				Offset: offset,
				Length: maxLength,
			})
			//fmt.Println(string(f.origin[int(offset+value):int(offset+value+length)]))
...
func (f *GoFile) GetTypes() ([]*GoType, error) {
	...
	core.TypeStringOffsets.Base, _ = f.fh.getFva(core.TypeStringOffsets.Base)
	m := sortTypes(t)
	//for _, k := range m {
	//	fmt.Println("type", k.String())
	//}
	return m, nil
}
