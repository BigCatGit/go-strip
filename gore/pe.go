...
func (p *peFile) getPCLNTab() (*gosym.Table, error) {
	...
	core.TypeStringOffsets.PCLnTab = p.pclntabAddr
...
func (p *peFile) getSectionDataFromOffset(off uint64) (uint64, []byte, error) {
	for _, section := range p.file.Sections {
		...
		if base+uint64(section.VirtualAddress) <= off && off < base+uint64(section.VirtualAddress+section.Size) {
			data, err := section.Data()
			return base + uint64(section.VirtualAddress), data, err
		}
	}
